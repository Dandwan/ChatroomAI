...
import { initRecorder, recordSession, finalizeRun } from './recorder.js';
import { compareAll } from './comparator.js';
import { printConsoleReport, writeMarkdownReport } from './reporter.js';
import { createLogger } from './logger.js';
import type { UserRequest, PendingSession, RunRecord } from './types.js';
...
async function main(): Promise<void> {
  const program = new Command()
    ...
    .option(
      ...
      "Which proxy's translated request to forward to real upstream: cpa or actinet",
      'cpa',
    )
  ...
  if (opts.forwardSource) {
    if (opts.forwardSource !== 'cpa' && opts.forwardSource !== 'actinet') {
      console.error(
        `Invalid forward source: ${opts.forwardSource}. Must be 'cpa' or 'actinet'.`,
      );
      process.exit(1);
    }
  ...
  try {
    if (opts.stdin) {
      ...
      console.error(
        'No request file specified. Provide a file path or use --stdin.',
      );
    ...
    console.error(
      `Failed to load user request: ${err instanceof Error ? err.message : err}`,
    );
    process.exit(1);
  }
  ...
  const simApp = createUpstreamSimulator(config, (tx) =>
  ...
  try {
    ...
    const reportPath = writeMarkdownReport(runRecord, outputDir);
