...
import type { ProxyDiffConfig } from './types.js'
...
function mergeConfig(defaults: ProxyDiffConfig, overrides: Partial<ProxyDiffConfig>): ProxyDiffConfig {
  return {
    cpa: { ...defaults.cpa, ...overrides.cpa },
    actiNet: { ...defaults.actiNet, ...overrides.actiNet },
    proxy: { ...defaults.proxy, ...overrides.proxy },
    realUpstream: { ...defaults.realUpstream, ...overrides.realUpstream },
    testSuite: { ...defaults.testSuite, ...overrides.testSuite },
  }
}
  ...
  for (const p of paths) {
    if (existsSync(p)) {
      try {
        ...
        console.error(`Warning: failed to load config from ${p}: ${err instanceof Error ? err.message : err}`)
      }
    }
  }
