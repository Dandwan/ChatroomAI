...
- a tag-based host protocol (`<progress>`, `<final>`, `<read>`, `<run>`)
...
- `src/services/skills/`
  - read/run protocol, skill host, runtime host, prompt defaults, native bridge
...
- `android/app/src/main/java/com/dandwan/chatroomai/SkillRuntimePlugin.java`
  - native runtime install/inspect/execute path, direct `run` execution, run session state
- `builtin-skills/`
