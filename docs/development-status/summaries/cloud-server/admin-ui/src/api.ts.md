...
## 功能
Admin UI API 客户端层。封装后端 `/api/admin/*` 端点的 HTTP 请求，自动 JWT token 注入和 401 重定向。定义前端类型和 CRUD 函数。**v5: 新增 `ServerSettings` 接口、`fetchSettings()`、`updateSettings()`；`UpstreamData` 使用 `key_fault_tolerance`；`api_keys` 数组项新增 `fault_tolerance`；新增 `UpstreamKeyData` 接口和 `updateUpstreamKey()`。**
