...
## 功能
Admin UI 根组件。包含 `AuthProvider` 上下文、登录状态检测、页面路由（dashboard/upstreams/model-priorities/users/api-keys/settings/logs/health）和条件渲染。未登录时显示 `LoginPage`，登录后通过 `Layout` 包裹对应页面。
