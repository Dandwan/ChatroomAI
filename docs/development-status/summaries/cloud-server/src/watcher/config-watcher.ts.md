...
## 功能
配置文件热重载监听器。使用 Node.js 内置 `fs.watch` 监听 `config.json` 变更，通过 500ms 防抖聚合快速连续写入，对比新旧配置后仅应用变更字段（logLevel、defaultFaultTolerance、actiNetModelMapping、proxyUrl、wsAuth、healthCheckIntervalMs）。端口、TLS 和 dbPath 变更需重启。
